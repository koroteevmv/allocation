        ...
        if 'stuffFile' in request.files and request.files['stuffFile']:
            ...
            if not model.list_stuff.equals(res.index.drop_duplicates()):
                ...
                msg = """При формировании матрицы предпочтений преподавателей обнаружено несоответствие.
                    ...
                    опечаток или разного написания имен преподавателей. В таком случае рекомендуем исправить соответствующие
                    названия в исходных файлах и перезагрузить таблицы нагрузки по преподавателям и\или предпочтений. Либо же это могут быть 
                    новые преподаватели. Тогда рекомендуем собрать по ним предпочтения. Важнее, чтобы левая колонка оказалась пуста."""
